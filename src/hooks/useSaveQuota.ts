...
const useUpdateClusterTemplateCosts = (): [
  ...
  const update = async (formValues: AllowedTemplateFormValues[]) => {
    const promises = [];
    for (const allowedTemplate of formValues) {
      ...
      if (clusterTemplate === undefined) {
        promises.push(
          Promise.reject(new Error(`Failed to find cluster template ${allowedTemplate.template}`)),
        );
...
const useSaveQuota = (
  ...
  const save = async (values: QuotaFormValues) => {
    let quota: Quota;
    if (originalQuota) {
