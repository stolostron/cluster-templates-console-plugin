...
export const useHelmRepositories = (): WatchK8sResult<
  HelmChartRepository[]
> => {
  const [repositories, loaded, loadError] = useK8sWatchResource<
    HelmChartRepository[]
  >({
  ...
  const templateRepositories = React.useMemo(
    () =>
      repositories.filter(
        ({ metadata }) => !!metadata?.labels?.[TEMPLATES_HELM_REPO_LABEL],
      ),
    [repositories],
  );
